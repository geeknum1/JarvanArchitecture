...
public class RouterInterceptor implements IInterceptor {
    ...
    public void process(Postcard postcard, InterceptorCallback callback) {
        ...
        if(postcard.getPath().contains(ARouterPath.NewsAty)){
            Log.d("mustang", "拦截到向ActivityGirls跳转");
            //自定义处理
           /* ARouter.getInstance()
                    .build(ARouterPath.GirlsAty)
                    .withString("params", "666")
                    .navigation(Utils.getContext());*/
        }else {
            Log.d("mustang", "非拦截跳转执行path: "+postcard.getPath());
        }
    ...
    public void init(Context context) {
        ...
        Log.d("mustang", "RouterInterceptor init");
    }
}
